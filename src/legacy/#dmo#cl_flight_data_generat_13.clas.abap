CLASS /dmo/cl_flight_data_generat_13 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    CLASS-METHODS:
      ...
      "!  <br/>
      "! Propagation to /dmo/cl_flight_legacy13=>calculate_flight_price.<br/>
      ...
      calculate_flight_price
        ...
        RETURNING
          VALUE(rv_price)           TYPE /dmo/flight_price13.
ENDCLASS.



CLASS /dmo/cl_flight_data_generat_13 IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.
    out->write( 'Starting Data Generation' ).

    out->write( 'Generate Data: Airport      /DMO/AIRPORT13' ).
    lcl_airport_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Carrier      /DMO/CARRIER13' ).
    lcl_carrier_data_generator=>lif_data_generator~create( out ).
    out->write( 'Generate Data: Connection   /DMO/CONNECTI_13' ).
    lcl_connection_data_generator=>lif_data_generator~create( out ).
    out->write( 'Generate Data: Flight       /DMO/FLIGHT13' ).
    lcl_flight_data_generator=>lif_data_generator~create( out ).


    out->write( 'Generate Data: Agency       /DMO/AGENCY13' ).
    lcl_agency_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Customer      /DMO/CUSTOMER13' ).
    lcl_customer_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Supplement      /DMO/SUPPLEME_13' ).
    lcl_supplement_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Travel      /DMO/TRAVEL13' ).
    out->write( 'Generate Data: Booking      /DMO/BOOKING13' ).
    out->write( 'Generate Data: Booking Supplement      /DMO/BOOK_SUP_13' ).
    lcl_travel_data_generator=>lif_data_generator~create( out ).

    out->write( 'Finished Data Generation' ).
  ENDMETHOD.
  METHOD calculate_flight_price.
    rv_price = /dmo/cl_flight_legacy13=>calculate_flight_price(
